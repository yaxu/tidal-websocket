...
-- We represent a client by their username and a `WS.Connection`. We
-- will see how we obtain this `WS.Connection` later on.

type Client = (Text, WS.Connection)

-- The state kept on the server is simply a list of connected
...
-- We also fork a pinging thread in the background. This will ensure
-- the connection stays alive on some browsers.

application state pending = do
  conn <- WS.acceptRequest pending
  WS.forkPingThread conn 30

-- When a client is succesfully connected, we read the first
-- message. This should be in the format of "Hi! I am Jasper", where
-- Jasper is the requested username.
...
-- Check the validity of the username:
           ...
           | any ($ fst client)
             [T.null, T.any isPunctuation, T.any isSpace] -> WS.sendTextData conn ("Name cannot contain punctuation or whitespace, and cannot be empty" :: Text)
...
-- We send a "Welcome!", according to our own little protocol. We add
-- the client to the list and broadcast the fact that he has
-- joined. Then, we give control to the 'talk' function.
